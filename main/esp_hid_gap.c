...
static void handle_ble_device_result(struct ble_scan_result_evt_param *scan_rst)
{
    ...
    uint8_t uuid_len = 0;
    uint8_t *uuid_d = esp_ble_resolve_adv_data(scan_rst->ble_adv, ESP_BLE_AD_TYPE_16SRV_PART, &uuid_len);
...
esp_err_t esp_hid_ble_gap_adv_init(uint16_t appearance, const char *device_name)
{
    ...
    if ((ret = esp_ble_gap_set_security_param(ESP_BLE_SM_IOCAP_MODE, &iocap, 1)) != ESP_OK) {
        ...
        ESP_LOGE(TAG, "GAP set_security_param IOCAP_MODE succeeded!: %d", ret);
    }
    ...
    */
    ...
    if ((ret = esp_ble_gap_set_device_name(device_name)) != ESP_OK) {
        ...
        ESP_LOGE(TAG, "GAP set_device_name(%s) succeeded: %d", device_name, ret);
    }
